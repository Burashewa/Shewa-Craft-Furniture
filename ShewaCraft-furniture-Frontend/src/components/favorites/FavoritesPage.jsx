...
export function FavoritesPage() {
  ...
  const handleRemove = async (productId) => {
    const favorite = favorites.find((item) => String(item.productId) === String(productId));
    try {
  ...
  const handleAddAllInStock = async () => {
    ...
    try {
      for (const favorite of inStockItems) {
        await addToCart({ product: resolveProduct(favorite), quantity: 1 });
      }
      showToast({
        type: 'success',
        title: 'Added to cart',
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
              ...
              <button
                ...
                className="px-4 py-2.5 border border-gray-900 rounded-md text-sm text-gray-900 hover:bg-gray-50 transition self-start sm:self-auto"
              >
