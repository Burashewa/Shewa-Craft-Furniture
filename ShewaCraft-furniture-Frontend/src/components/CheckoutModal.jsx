...
import { createOrder, listBanks } from '../services/orderService';
...
export function CheckoutModal({ cartItems, total, onClose, onPlaced }) {
  const { showToast } = useToast();
  const [bankAccounts, setBankAccounts] = useState(FALLBACK_BANKS);
  ...
  const [placedTotal, setPlacedTotal] = useState(Number(total || 0));

  useEffect(() => {
    let active = true;
    listBanks()
      .then((banks) => {
        if (active && banks.length > 0) setBankAccounts(banks);
      })
      .catch(() => {});
    return () => {
      active = false;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setPlacedTotal(Number(created?.price ?? total ?? 0));
      setIsSubmitted(true);
      await onPlaced?.();
    } catch (err) {
      const fieldErrors = err.fields || {};
      setErrors({
        bank: fieldErrors.bank,
      ...
      showToast({
        ...
        message: err.message || 'Unable to submit your order.',
      });
    } finally {
        ...
        <div className="bg-white border border-gray-200 rounded-xl max-w-md w-full p-8">
          <div className="text-center">
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-lg p-4 mb-6">
              ...
              <p className="text-lg text-gray-900 mb-3">{orderId}</p>
              <p className="text-xs uppercase tracking-wider text-gray-500 mb-1">Order total</p>
              <p className="text-3xl text-gray-900">${Number(placedTotal || 0).toFixed(2)}</p>
            </div>
