...
function productWriteBody(product) {
  return {
    name: product.name,
    price: product.price,
...
export function ProductsManagement({
  ...
  const syncCatalog = async () => {
    try {
      ...
      // Admin inventory already saved; storefront refresh is best-effort.
    }
  };
  ...
  const replaceProduct = (product) => {
    ...
    setProducts((prev) => {
      const exists = prev.some((item) => String(item.id) === String(next.id));
      if (exists) {
        return prev.map((item) =>
          String(item.id) === String(next.id) ? next : item
        );
      }
      return [...prev, next];
    });
  ...
  const handleImagesChange = async (e) => {
    ...
    try {
      ...
      showToast({
        type: 'error',
        title: 'Upload failed',
        message: err.message || 'One or more photos could not be uploaded. Please try again.',
      });
    } finally {
      setUploading(false);
      e.target.value = '';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const saved = editingProduct
        ? await updateAdminProduct(editingProduct.id, productWriteBody(payload))
        : await createAdminProduct(productWriteBody(payload));
      const next = replaceProduct(saved);
      await syncCatalog();
      showToast({
        type: 'success',
        title: editingProduct ? 'Product updated' : 'Product added',
        message: editingProduct
          ? `${next.name} was updated successfully.`
          : `${next.name} was added to your inventory.`,
      });
      setShowFormModal(false);
    } catch (err) {
      showToast({
        type: 'error',
        title: editingProduct ? 'Update failed' : 'Could not add product',
  ...
  const handleQuickStockToggle = async (product) => {
    ...
    try {
      const saved = await updateAdminProductStock(product.id, {
        inStock: nextInStock,
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="space-y-3">
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-900 disabled:opacity-60"
                    />
              ...
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                ...
                <button
                  ...
                  className="px-6 py-2 bg-gray-900 text-white rounded-md hover:bg-gray-800 transition disabled:opacity-60"
                >
                  {saving
                    ? 'Saving…'
                    : editingProduct
